...
describe('ToastManager', () => {
  ...
  it('success() calls show with type success', () => {
    ...
    expect(spy).toHaveBeenCalledWith({ message: 'Success!', type: 'success', duration: undefined });
  });

  it('error() calls show with type error', () => {
    const spy = vi.spyOn(toast, 'show');
    toast.error('Error!');
    expect(spy).toHaveBeenCalledWith({ message: 'Error!', type: 'error', duration: undefined });
  });

  it('info() calls show with type info', () => {
    const spy = vi.spyOn(toast, 'show');
    toast.info('Info!');
    expect(spy).toHaveBeenCalledWith({ message: 'Info!', type: 'info', duration: undefined });
  });

  it('warning() calls show with type warning', () => {
    const spy = vi.spyOn(toast, 'show');
    toast.warning('Warning!');
    expect(spy).toHaveBeenCalledWith({ message: 'Warning!', type: 'warning', duration: undefined });
  });
